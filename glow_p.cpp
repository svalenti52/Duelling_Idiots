...
//	bool whole_swit_close = true;
	...
	{
		std::vector<bool> parallel_glows;

		for ( int ix = 0; ix < nr_swit_per_dim; ++ix ) // parallel for outermost
		{
			...
			if ( parallel_glows )
				parallel_glows.push_back(true);
			else
				parallel_glows.push_back(false);
		}

		if ( std::any_of(parallel_glows.begin(), parallel_glows.end(), [](bool val) {return val;}) )
			++nr_glows;
	}
